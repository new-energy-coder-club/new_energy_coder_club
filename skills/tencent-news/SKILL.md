...
3. **每次会话开始时**，自动获取并配置 API Key：
   ...
   - Windows PowerShell：
   ```powershell
   $apiKey = & "<SCRIPT_PATH>\get-token.ps1"; bun "<SCRIPT_PATH>\scripts\run-cli.ts" apikey-set $apiKey
   ```
4. 若脚本报错，提示用户在**应用内集成面板**中完成腾讯新闻授权（不要引导用户手动去网页获取 Key）
...
**优先自动获取**：读取同目录下的 `SETUP_TOKEN.md`，将 `<SCRIPT_PATH>` 替换为本文件所在目录的绝对路径，然后执行：
...
手动设置 Key（通过 `run-cli` 执行，KEY 是裸值不加引号）：
...
- Token 自动获取说明：[`SETUP_TOKEN.md`](SETUP_TOKEN.md)
