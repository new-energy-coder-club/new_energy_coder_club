#!/usr/bin/env bash
# get-token.sh — 从凭证托管服务获取腾讯新闻 API Key
#
# 用法:
#   api_key=$(bash get-token.sh)
#   sh scripts/run-cli.sh apikey-set "$api_key"
#
# Token 由本地代理服务自动注入 JWT，无需手动传入

set -euo pipefail

# 确保 UTF-8 locale（防止某些精简 Linux 环境 LANG=C 导致中文截断/乱码）
...
PLATFORM="${CREDENTIAL_PLATFORM:-tencent_news}"
...
if [[ "$HTTP_STATUS" != "200" ]]; then
  echo "ERROR: HTTP ${HTTP_STATUS}，请先在集成面板中完成腾讯新闻授权" >&2
  exit 1
fi
...
if [[ "$ret" != "0" ]]; then
  echo "ERROR: ret=${ret}，请先在集成面板中完成腾讯新闻授权" >&2
  exit 1
fi

# 提取 access_token（即 API Key）
access_token=$(json_extract "$response" '.data.resp.data.access_token')

if [[ -z "$access_token" || "$access_token" == "null" ]]; then
  echo "ERROR: 未获取到 API Key，请先在集成面板中完成腾讯新闻授权" >&2
  exit 1
fi
