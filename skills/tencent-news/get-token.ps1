# get-token.ps1 — 从凭证托管服务获取腾讯新闻 API Key
#
# 用法:
...
if ($response.ret -ne 0) {
    [Console]::Error.WriteLine("ERROR: ret=$($response.ret)，请先在集成面板中完成腾讯新闻授权")
    exit 1
}
...
if (-not $accessToken -or $accessToken -eq "null") {
    [Console]::Error.WriteLine("ERROR: 未获取到 API Key，请先在集成面板中完成腾讯新闻授权")
    exit 1
}
