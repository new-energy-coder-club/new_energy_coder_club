# get-token.ps1 — 从凭证托管服务获取携程问道 API Key
#
# 用法:
#   $token = & .\get-token.ps1
#   $env:WENDAO_API_KEY = $token
#
# Token 由本地代理服务自动注入 JWT，无需手动传入

$ErrorActionPreference = "Stop"

$Platform  = if ($env:CREDENTIAL_PLATFORM) { $env:CREDENTIAL_PLATFORM } else { "wendao" }
...
if ($response.ret -ne 0) {
    [Console]::Error.WriteLine("ERROR: ret=$($response.ret)，请先在集成面板中完成携程问道授权")
    exit 1
}
...
if (-not $accessToken -or $accessToken -eq "null") {
    [Console]::Error.WriteLine("ERROR: 未获取到 access_token，请先在集成面板中完成携程问道授权")
    exit 1
}
