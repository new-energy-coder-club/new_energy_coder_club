...
# ── 计算 MCP 请求签名 ─────────────────────────────────────────────────────────
# 规则：X-MCP-Request-Sign = <产品标识(1位)> + SHA256(token + 产品名称)
# 当前产品：qclaw → 标识 "q"
# 用法：_tdoc_build_request_sign <token>
_tdoc_build_request_sign() {
    local token="$1"
    local product_flag="q"
    local product_name="qclaw"
    local payload="${token}${product_name}"

    local hash=""
    # 优先使用 openssl，其次 sha256sum / shasum，保持与脚本其它处一致的 fallback 风格
    if command -v openssl &>/dev/null; then
        hash=$(printf '%s' "$payload" | openssl dgst -sha256 2>/dev/null | awk '{print $NF}')
    elif command -v sha256sum &>/dev/null; then
        hash=$(printf '%s' "$payload" | sha256sum 2>/dev/null | awk '{print $1}')
    elif command -v shasum &>/dev/null; then
        hash=$(printf '%s' "$payload" | shasum -a 256 2>/dev/null | awk '{print $1}')
    fi

    [[ -z "$hash" ]] && return 1
    echo "${product_flag}${hash}"
}
...
_tdoc_save_token() {
    ...
    # 计算请求签名（绑定当前 token）
    local sign
    sign=$(_tdoc_build_request_sign "$token") || {
        echo "ERROR:sign_failed"
        return 1
    }

    # 先移除旧配置，确保 config add 能写入完整 header（mcporter config add 不会覆盖已有服务）
    mcporter config remove "$_TDOC_SERVICE_NAME" 2>/dev/null || true
    mcporter config remove tencent-docengine 2>/dev/null || true
    mcporter config remove tencent-sheetengine 2>/dev/null || true

    # 使用传入的 token 写入 mcporter 配置（tencent-docs）
    mcporter config add "$_TDOC_SERVICE_NAME" "$_TDOC_MCP_URL" \
        --header "Authorization=$token" \
        --header "X-MCP-Request-Sign=$sign" \
        --transport http \
        --scope home
