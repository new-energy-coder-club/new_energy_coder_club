# get-token.ps1 — 从凭证托管服务获取 Notion access_token
#
# 用法:
#   $token = & .\get-token.ps1          # 变量捕获
#   $env:NOTION_TOKEN = & .\get-token.ps1  # 环境变量赋值
#
# Token 由本地代理服务自动注入 JWT，无需手动传入

$ErrorActionPreference = "Stop"

$Platform  = if ($env:CREDENTIAL_PLATFORM) { $env:CREDENTIAL_PLATFORM } else { "notion" }
...
if (-not $accessToken -or $accessToken -eq "null") {
    [Console]::Error.WriteLine("ERROR: 未获取到 access_token，请先在集成面板中完成 Notion 授权")
    exit 1
}
