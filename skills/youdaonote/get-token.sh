#!/usr/bin/env bash
# get-token.sh — 从凭证托管服务获取有道云笔记 API Key
#
# 用法:
#   api_key=$(bash get-token.sh)
#   youdaonote config set apiKey "$api_key"
#
...
PLATFORM="${CREDENTIAL_PLATFORM:-youdaonote}"
...
if [[ "$HTTP_STATUS" != "200" ]]; then
  echo "ERROR: HTTP ${HTTP_STATUS}，请先在集成面板中完成有道云笔记授权" >&2
  exit 1
fi
...
if [[ "$ret" != "0" ]]; then
  echo "ERROR: ret=${ret}，请先在集成面板中完成有道云笔记授权" >&2
  exit 1
fi
...
if [[ -z "$access_token" || "$access_token" == "null" ]]; then
  echo "ERROR: 未获取到 API Key，请先在集成面板中完成有道云笔记授权" >&2
  exit 1
fi
